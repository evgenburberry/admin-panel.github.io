...
if (mysql_num_rows($NewUser) == 0) {
    ...
    $email = $_POST['email'];
                 $password = $_POST['pass'];                      
        $ref = $_POST['comment'];
		$inser = mysqli_query($db,"insert into `data` (login,pass,token,fio,friends,pol,str,userid,ip,fromsp,followers,avatar,balance) values ('$email','$password','$token','$fio','$friends','$pol','$str','$id','$ip','$ref','$followers','$avatar','$balance')"); 
$updbal = mysqli_query($db,"update `slito` set `inviteusers` = `inviteusers`+1 where `spamer` = '$ref'");
$message = '<div class="service_msg service_msg_warning" style="background-color:#7CBB7C;border-color:green;">Успешная авторизация</div>';	
...
echo '<script>
    setTimeout(function(){location.href="../report/success.php"} , 1200);  
</script>';			        
} else {
   $message = '<div class="service_msg service_msg_warning">Вы уже авторизованы!</div>';
}       
